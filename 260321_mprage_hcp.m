...
%   matlab -batch "run('260321_mprage_hcp.m')"
...
ro_dur = N_ro * ro_os / (BW_per_pixel * N_ro);  % = ro_os / BW [s]
...
%% Pulse and gradient construction (shared between both variants)

% Excitation: hard pulse
rf = mr.makeBlockPulse(alpha*pi/180, sys, 'Duration', rfLen, 'use', 'excitation');

% Inversion: adiabatic hyperbolic secant
...
% Readout prewinder (accounts for Siemens centre-of-dwell sampling)
groPre = mr.makeTrapezoid(ax.d1, ...
    'Area', -gro.amplitude*(adc.dwell*(adc.numSamples/2 + 0.5) + 0.5*gro.riseTime), ...
    'system', sys);

% Phase-encoding gradients (maximum amplitudes)
...
% Spoiler along partition direction (4 cycles per voxel)
gslSp = mr.makeTrapezoid(ax.d3, 'Area', max(deltak.*N)*4, 'Duration', 10e-3, 'system', sys);

% Split readout gradient for spoiler overlap
...
% Inner-loop timing: block 1 = [spoilers/rewinders + RF], block 2 = [prewinder + readout]
...
% Phase-encoding step tables (normalised to [-1, 1])
pe1Steps = ((0:N(ax.n2)-1) - N(ax.n2)/2) / N(ax.n2) * 2;  % partition
pe2Steps = ((0:N(ax.n3)-1) - N(ax.n3)/2) / N(ax.n3) * 2;  % phase

% TI delay: from centre of inversion pulse to centre of excitation at k-space centre
TIdelay = round((TI ...
